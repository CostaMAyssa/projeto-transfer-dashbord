-- Migração Inicial - AZ Transfer Booking
-- Data: 2024-01-20
-- Descrição: Criação do esquema inicial com suporte a dados de voos
...
create index if not exists idx_flight_data_airports on flight_data(origin_airport, destination_airport);
create index if not exists idx_flight_data_departure on flight_data(scheduled_departure);
...
create index if not exists idx_bookings_flight_data on bookings(flight_data_id);
